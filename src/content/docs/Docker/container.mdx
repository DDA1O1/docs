...
* Define your application's environment in a `compose.yaml` file
* Manage multiple containers as a single service
* Maintain container relationships and dependencies
...
Here's a complete example of a web application with multiple services:
...
A Dockerfile is a text document containing instructions to build a Docker image. Understanding how to write an efficient Dockerfile is crucial for creating optimized container images.
  ...
  <ol>
    <li>
      **FROM**
      ...
      FROM <image>:<tag>
      ```
      Specifies the base image. Always start with this instruction.
    </li>
    <li>
      **WORKDIR**
      ```dockerfile
      ...
      ```
      Sets the working directory for subsequent instructions. Use absolute paths.
    </li>
    <li>
      **COPY and ADD**
      ```dockerfile
      COPY <src> <dest>
      ADD <src> <dest>  # Can handle URLs and auto-extract archives
      ```
      Copies files from host to container. Prefer COPY unless you need ADD's special features.
    </li>
    <li>
      **RUN**
      ```dockerfile
      RUN <command>
      ```
      Executes commands in a new layer. Combine multiple commands with && to reduce layers.
    </li>
    <li>
      **ENV**
      ```dockerfile
    ...
    <li>
      **CMD**
      ...
      ```
      Provides defaults for executing container. Only one CMD instruction is effective.
    </li>
  </ol>
</Steps>

### Understanding Layer Optimization

<Aside type="tip" title="Docker Image Layers">
Each instruction in a Dockerfile creates a new layer in the image. Layers are cached and reused, which can significantly impact build time and image size.
</Aside>
...
```dockerfile
# Good - Less frequently changed files first
...
The `.dockerignore` file helps exclude files and directories from the build context, improving build performance and security.
...
Always use `.dockerignore` to prevent sensitive files (like `.env`, `.git`, and encryption keys) from being copied into your image.
</Aside>
...
<Aside type="note" title="Layer Caching">
Docker caches each layer during the build process. When a layer changes, all subsequent layers must be rebuilt. This is why ordering instructions by change frequency is crucial for build performance.
</Aside>
