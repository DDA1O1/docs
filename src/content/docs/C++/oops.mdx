...
int main() {
    // Initialize vector
    vector<int> nums = {1, 2, 3};              // With values
    vector<string> names(3);                    // Size of 3
    vector<double> prices(5, 0.0);             // Size 5, all set to 0.0

    // Basic operations
    nums.push_back(4);                         // Add to end: O(1)
    nums.pop_back();                           // Remove from end: O(1)
    nums[0] = 10;                              // Direct access: O(1)
    
    // Useful methods
    int size = nums.size();                    // Current size
    bool empty = nums.empty();                 // Check if empty
    nums.clear();                              // Remove all elements
    
    // Iteration
    vector<int> items = {1, 2, 3, 4, 5};
    for(int item : items) {                    // Range-based loop
        cout << item << " ";
    }
    
    // Common operations
    items.insert(items.begin(), 0);            // Insert at start: O(n)
    items.erase(items.begin());                // Remove from start: O(n)
    items.resize(10);                          // Resize vector
...
- Slow insertion/removal at beginning/middle: O(n)
...
int main() {
    // Initialize list
    list<int> nums = {1, 2, 3};                // With values
    list<string> names(3, "unknown");          // Size 3, all "unknown"

    // Basic operations
    nums.push_back(4);                         // Add to end: O(1)
    nums.push_front(0);                        // Add to front: O(1)
    nums.pop_back();                           // Remove from end: O(1)
    nums.pop_front();                          // Remove from front: O(1)
    ...
    // Common operations
    auto it = items.begin();
    advance(it, 2);                            // Move iterator to position
    items.insert(it, 10);                      // Insert at position: O(1)
    items.erase(it);                           // Remove at position: O(1)
...
- Elements stored in multiple chunks of contiguous memory
- Provides random access like vector
- More flexible than vector but slightly slower for random access
...
int main() {
    // Initialize deque
    deque<int> nums = {1, 2, 3};              // With values
    deque<string> names(3, "unknown");         // Size 3, all "unknown"
    ...
    nums.pop_back();                           // Remove from end: O(1)
    nums.pop_front();                          // Remove from front: O(1)
    nums[1] = 10;                              // Random access: O(1)

    // Size operations
    int size = nums.size();                    // Get size
    bool empty = nums.empty();                 // Check if empty
    nums.resize(5);                            // Resize deque
    nums.clear();                              // Remove all elements

    // Iteration
    deque<int> items = {1, 2, 3, 4, 5};
    for(const auto& item : items) {            // Range-based loop
        cout << item << " ";
    }

    // Insert/Remove operations
    auto it = items.begin() + 2;               // Iterator to position
    items.insert(it, 10);                      // Insert at position
    items.erase(it);                           // Remove at position

    // Access elements
    int first = items.front();                 // First element
    int last = items.back();                   // Last element
    int third = items.at(2);                   // Element at index 2
...
- Memory stored in chunks, not fully contiguous
...
- Better than vector when frequent push_front needed
- Better than list when random access needed
...
int main() {
    ...
    orders.push_back({4, 104, 3, time(nullptr), "C004", "Processing"});
    orders.push_back({5, 105, 2, time(nullptr), "C005", "Processing"});
    orders.push_back({6, 106, 1, time(nullptr), "C006", "Processing"});
    orders.push_back({7, 107, 1, time(nullptr), "C007", "Processing"});
    ...
    // `vector<Product>` is the value type

    for (const auto& product : products) {
        productsByCategory[product.Category].push_back(product); // insert the product into the vector of products by category
    }

    for (const auto& [category, products] : productsByCategory) {
        cout << category << endl;
        for (const auto& product : products) {
    ...
    unordered_map<string, vector<Order>> ordersByCustomer;
    for (const auto& order : orders) {
        ordersByCustomer[order.CustomerID].push_back(order); // insert the order into the vector of orders by customer
    }

    unordered_set<string> uniqueCustomers;
    for (const auto& order : orders) {
        uniqueCustomers.insert(order.CustomerID); // insert the customer ID into the unordered set
    }
