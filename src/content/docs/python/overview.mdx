...
A function is a reusable block of code that performs a specific task. Functions are defined using the `def` keyword. They help organize code and promote reuse.

```python:src/content/docs/python/functions.mdx
def greet(name):
...
Docstrings provide inline documentation for your functions. Placed immediately after the function definition, they explain the function’s purpose, its parameters, return values, and any exceptions raised.
...
You can use the built-in `help()` function to view a function’s docstring:
...
- **Positional Arguments:** Required and passed in order.
- **Default Arguments:** These have default values and are used when no value is provided.
- **Variable-length Arguments:** Use `*args` for extra positional arguments and `**kwargs` for extra keyword arguments.

```python:src/content/docs/python/functions.mdx
def calculate_sum(a, b=0, *args, **kwargs):
    """Calculate the sum of multiple numbers.

    Args:
        a (int): The first number.
        b (int, optional): The second number. Defaults to 0.
        *args (int): Additional positional numbers.
        **kwargs (int): Additional numbers provided as keyword arguments.
...
# Examples:
# calculate_sum(1, 2) returns 3
# calculate_sum(1, 2, 3, 4, num1=5, num2=6) returns 21
```

## Anonymous Functions

Lambda functions, or anonymous functions, provide a concise way to write small functions without formally defining them.
...
Use the built-in `help()` function to display a function’s documentation. This is particularly useful for quickly reviewing the usage of a function.
...
- **Docstrings** serve as inline documentation to explain what each function does, its parameters, and its return value.
- **Parameter Types** such as positional, default, and variable-length add flexibility.
- **Lambda Functions** simplify the definition of small, one-off functions.
- The **`help()` Function** allows users to view documentation quickly.

---

## Object-Oriented Programming in Python

OOP is a paradigm that organizes code using classes and objects. In Python, classes provide the structure to encapsulate data (attributes) and behavior (methods).

### Creating Classes in Python

A class in Python is defined using the `class` keyword. The constructor-like method, `__init__`, initializes new instances, and the first parameter `self` refers explicitly to the current instance.

```python:src/content/docs/python/oops.mdx
class Duck:
    def __init__(self, name):
        """Initialize a new Duck instance.

        Args:
            name (str): The name of the duck.
        """
        self.name = name       # Similar to 'this.name' in JavaScript
        self.sound = "Quack"
...
# Create an instance of Duck and use its method
donald = Duck("Donald")
...
Inheritance enables a child class to reuse code from a parent class. Python uses the `super()` function to call the parent class's initializer.

```python:src/content/docs/python/oops.mdx
class Duckling(Duck):
    def __init__(self, name):
        """Initialize a Duckling instance inheriting from Duck.

        Args:
            name (str): The name of the duckling.
        """
        super().__init__(name)  # Calls Duck.__init__(name)
        self.size = "small"

donnie = Duckling("Donnie")
donnie.quack()  # Output: Quack
```

## JavaScript vs. Python: Constructors and the `this`/`self` Context

Both languages support OOP but differ in syntax and naming conventions for initialization.

### JavaScript Approach

In JavaScript, the `constructor` method initializes new objects, and the `this` keyword is used implicitly to refer to the current instance.
...
Python uses the `__init__` method for initialization, and every instance method must explicitly include `self` to reference the object.

```python
# Python Example (see above)
class Duck:
    def __init__(self, name):
        self.name = name     # 'self' is the explicit reference to the instance
        self.sound = "Quack"
...
- **Context Reference:**  
  - **JavaScript:** Uses `this` implicitly.  
  - **Python:** Requires `self` as the first parameter in instance methods.

- **Constructor Naming:**  
  - **JavaScript:** The constructor method is named `constructor`.  
  - **Python:** The initializer method is named `__init__`.

## Summary

- **Classes** encapsulate data and behaviors.
- **Inheritance** in Python allows for code reuse via `super()`.
- **`this` vs. `self`:**  
  - JavaScript uses `this` implicitly in class methods.
  - Python uses an explicit `self` parameter.
- **Constructor Functions:**  
  - JavaScript defines constructors with the `constructor` method.
  - Python initializes objects using the `__init__` method.
