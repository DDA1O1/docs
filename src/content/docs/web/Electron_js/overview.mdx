...
6.  **Automating Releases with GitHub Actions**
    ...
    **Setting up GitHub Token for Publishing**
    ...
    build-windows-x64:
        runs-on: windows-latest # Use a Windows runner for Squirrel/NSIS/EXE build
        steps:
        - name: Checkout Repository
            uses: actions/checkout@v4
            with:
            fetch-depth: 0 # Needed for version detection by some tools
        ...
        # Explicitly make for win32 x64 using the configured maker(s)
    ...
    build-windows-arm64:
        ...
        # Explicitly make for win32 arm64 using the configured maker(s)
    ...
    build-linux:
        runs-on: ubuntu-latest # Use a Linux runner for Deb/RPM builds
        steps:
        - name: Checkout Repository
            uses: actions/checkout@v4
            with:
        ...
        - name: Install Linux Build Dependencies
            run: |
            sudo apt-get update
            sudo apt-get install -y --no-install-recommends dpkg fakeroot # For .deb
            # Add other dependencies if using maker-rpm (e.g., rpmbuild)

        - name: Install Project Dependencies
            run: npm ci

        # Explicitly make for linux x64 using the configured Deb/RPM maker(s)
        - name: Build Linux x64 (Make)
            run: npm run make -- --platform=linux --arch=x64

        # Explicitly make for linux arm64 using the configured Deb/RPM maker(s)
        - name: Build Linux arm64 (Make)
            run: npm run make -- --platform=linux --arch=arm64

        # Upload the contents of the make output directory (contains both .deb/.rpm files)
    ...
    release:
        ...
        - name: Download Windows x64 Artifact
            ...
            path: staging/windows-x64 # Contains maker output (e.g., squirrel.windows/x64 or nsis/x64)
        ...
        - name: Download Windows arm64 Artifact
            ...
            path: staging/windows-arm64 # Contains maker output (e.g., squirrel.windows/arm64 or nsis/arm64)
        ...
        - name: Download Linux Artifacts
            ...
            with:
            name: linux-artifacts
            path: staging/linux # Contains maker output (e.g., deb/x64, deb/arm64)

        # --- Optional Renaming Steps (Adjust paths based on your makers) ---
        # Example for Squirrel.Windows:
        - name: Rename Windows x64 Squirrel Artifacts for Uniqueness
          if: ${{ hashFiles('staging/windows-x64/squirrel.windows/x64/*') != '' }} # Only run if files exist
          ...
          run: |
            ...
            for FILE in *; do
              if [[ -f "$FILE" ]]; then
                ...
                [[ "$FILENAME" == "$EXTENSION" ]] || [[ "$FILENAME" == "" ]] && NEW_NAME="${FILE}-x64" || NEW_NAME="${FILENAME}-x64.${EXTENSION}"
                echo "Renaming '$FILE' to '$NEW_NAME'"; mv "$FILE" "$NEW_NAME"
              fi
            done
            echo "--- Files after renaming in $(pwd) ---"; ls -l

        - name: Rename Windows arm64 Squirrel Artifacts for Uniqueness
          if: ${{ hashFiles('staging/windows-arm64/squirrel.windows/arm64/*') != '' }} # Only run if files exist
          ...
          run: |
            ...
            for FILE in *; do
              if [[ -f "$FILE" ]]; then
                FILENAME="${FILE%.*}"; EXTENSION="${FILE##*.}"
                [[ "$FILENAME" == "$EXTENSION" ]] || [[ "$FILENAME" == "" ]] && NEW_NAME="${FILE}-arm64" || NEW_NAME="${FILENAME}-arm64.${EXTENSION}"
                echo "Renaming '$FILE' to '$NEW_NAME'"; mv "$FILE" "$NEW_NAME"
              fi
            done
            echo "--- Files after renaming in $(pwd) ---"; ls -l

        # Example for NSIS (NSIS usually creates uniquely named setup files, but you might want to standardize)
        # - name: Rename Windows x64 NSIS Artifact for Uniqueness
        #   if: ${{ hashFiles('staging/windows-x64/nsis/x64/*') != '' }}
        #   working-directory: staging/windows-x64/nsis/x64
        #   run: |
        #      # Find the setup exe (adjust pattern if needed) and rename it
        #      SETUP_EXE=$(ls *.exe | head -n 1)
        #      if [[ -n "$SETUP_EXE" ]]; then
        #         NEW_NAME="YourApp-Setup-${{ github.ref_name }}-x64.exe"
        #         echo "Renaming '$SETUP_EXE' to '$NEW_NAME'"
        #         mv "$SETUP_EXE" "$NEW_NAME"
        #      fi
        # Similar step for NSIS arm64 if needed...

        # --- End of Optional Renaming Steps ---
        ...
        - name: Create GitHub Release and Upload Artifacts
            ...
            # The recursive glob should find the files, potentially renamed, in their nested paths
            files: staging/**/*.* # Be more specific to avoid uploading empty dirs
            env:
